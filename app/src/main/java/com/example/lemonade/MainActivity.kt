...
class MainActivity : AppCompatActivity() {
     ...
     */
    //method that will be clicked when user clicks on image
    private fun clickLemonImage() {
        // TODO: use a conditional statement like 'if' or 'when' to track the lemonadeState
        when (lemonadeState) {
            SELECT -> {
                //if the state is SELECT that means on tap we will try to squeeze the lemon to set the state to SQUEEZE
                lemonadeState = SQUEEZE

                //determining the size of the lemon by pick() function
            ...
            SQUEEZE -> {
                //now we are squeezing so count will be increased by 1 and the size will be decreased by 1
                //squeezeCount++
                squeezeCount += 1
                //lemonSize--
                lemonSize -= 1
                //now the lemonadeState will be in squeeze state until we do not complete the whole lemon
                //(lemonSize = 0) so if the lemonSize is 0, then the state will be DRINK till then we have to squeeze
            ...
            //restart means now we will have to select the lemon from the tree
    ...
    private fun setViewElements() {
        ...
        //whatever the state is we have to set properties of image and text as follows
        when(lemonadeState){
            SELECT -> {
            ...
            RESTART -> {
                textAction.text = resources.getString(R.string.lemon_restart)
